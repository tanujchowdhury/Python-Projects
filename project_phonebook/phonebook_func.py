...
# create database db_phonebook.db
def create_db(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute(
                ...
                ?)""",
                ('Tanuj', 'Chowdhury', 'Tanuj Chowdhury', '510-676-5504', 'chowdhury.tanuj@gmail.com'))
            conn.commit()
    conn.close()
...
def onSelect(self, event):
    ...
    with conn:
        ...
        # This returns a tuple and is sliced into 4 parts using data[] during the iteration
        for data in varBody:
...
def addToList(self):
    ...
    # combine normalized names into a fullname
    ...
    # enforce the user to provide both names
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0):
        ...
        with conn:
            ...
            cursor.execute("""SELECT COUNT(col_fullname) FROM tbl_phonebook WHERE col_fullname = '{}'""".format(
...
def onDelete(self):
    ...
    with conn:
        ...
        # cannot delete last record, or it will throw an error
        cur.execute("""SELECT COUNT(*) FROM tbl_phonebook""")
        count = cur.fetchone()[0]
        if count > 1:
            confirm = messagebox.askokcancel("Delete Confirmation",
                                             "All information associated with, ({}) \nwill be permanently deleted "
                                             "from the database. \n\nProceed with the deletion request?".format(
                                                 var_select))
            if confirm:
                conn = sqlite3.connect('db_phonebook.db')
                with conn:
                ...
                onDeleted(self)  # call the function to clear all the text-boxes and the selected index of listbox
                # onRefresh(self) # update the listbox of the changes
                conn.commit()
        else:
            confirm = messagebox.showerror("Last Record Error",
                                           "({}) is the last record in the database and cannot be deleted at this "
                                           "time. \n\nPlease add another record first before you can delete ({"
                                           "}).".format(
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0):  # ensure that there is data present
        ...
        with conn:
            ...
            # so there are no changes to update.
            ...
            if count == 0 or count2 == 0:  # if proposed changes are not already in the database, then proceed
                ...
                if response:
                    ...
                    with conn:
                        cursor = conn.cursor()
                        cursor.execute(
                            """UPDATE tbl_phonebook SET col_phone = '{0}',col_email = '{1}' WHERE col_fullname = '{
                            2}'""".format(
                                var_phone, var_email, var_value))
                        onClear(self)
                        conn.commit()
